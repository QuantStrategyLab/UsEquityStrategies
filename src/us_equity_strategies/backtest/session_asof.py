...
class SessionClose:
    ...
    def __post_init__(self) -> None:
        ...
        if parsed.astimezone(US_MARKET_ZONE).date() != trading_date:
...
def build_close_snapshot(session: SessionClose, *, total_equity: float, cash_balance: float, positions: tuple[Position, ...] = ()) -> PortfolioSnapshot:
    """Build a planner snapshot stamped with the validated US session close."""
    return PortfolioSnapshot(as_of=session.close_datetime, total_equity=float(total_equity), buying_power=float(cash_balance), cash_balance=float(cash_balance), positions=positions, metadata={"trading_date": session.trading_date.isoformat()})
