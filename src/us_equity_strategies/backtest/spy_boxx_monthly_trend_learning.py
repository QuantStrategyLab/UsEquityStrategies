...
class SpyBoxxMonthlyTrendLearningRunner:
    ...
    def _simulate(
        ...
        initial_signal_position = start_index - 1
        while initial_signal_position >= SMA_SESSIONS and not _is_first_eligible_session(
            self._close.index, initial_signal_position
        ):
            initial_signal_position -= 1
        if initial_signal_position < SMA_SESSIONS:
            raise ValueError("OOS fold requires a causally available monthly signal")
        initial_decision = _monthly_decision_from_close(
            self._close,
            as_of=self._close.index[initial_signal_position],
            account_equity=completed_equity,
            drawdown_scalar=1.0,
        )
        if initial_decision.positions:
            active_symbol = initial_decision.positions[0].symbol
            initial_weight = float(
                initial_decision.positions[0].target_weight or 0.0
            )
            entry_price = float(self._close[active_symbol].iloc[start_index - 1])
            units = initial_weight / entry_price
            cash -= initial_weight
            max_effective_exposure = initial_weight * PRODUCT_LEVERAGE_FACTORS.get(
                active_symbol, 0
            )
        else:
            daily_rejects.iloc[0] = 1

        for index_position in range(start_index, end_index + 1):
            as_of = self._close.index[index_position]
            starting_equity = completed_equity
            traded_notional = 0.0
            ...
            if active_symbol is not None:
                ...
                unslipped_fill: float | None = None
                if open_price <= stop_price:
                    unslipped_fill = open_price
                    stop_gap_open_events += 1
                elif low_price <= stop_price:
                    unslipped_fill = stop_price
                if unslipped_fill is not None:
                    fill_price = unslipped_fill * (
                        1.0 - STOP_SLIPPAGE_BPS / 10_000.0
                    )
                    sale_notional = units * fill_price
                    sale_cost = sale_notional * COST_PER_SIDE_BPS / 10_000.0
                    cash += sale_notional - sale_cost
                    traded_notional += sale_notional
                    transaction_cost += sale_cost
                    stop_events += 1
                    stop_slippage_costs += units * (
                        unslipped_fill - fill_price
                    ) / starting_equity
                    units = 0.0
                    active_symbol = None
                    entry_price = None
            ...
            position_value = (
                units * float(self._close[active_symbol].iloc[index_position])
                if active_symbol is not None
                else 0.0
            )
            current_equity = cash + position_value
            current_peak = max(peak_completed_equity, current_equity)
            scalar = _drawdown_scalar(1.0 - current_equity / current_peak)
            if scalar == 0.0:
                drawdown_breaker = True

            if drawdown_breaker and active_symbol is not None:
                sale_notional = position_value
                sale_cost = sale_notional * COST_PER_SIDE_BPS / 10_000.0
                cash += sale_notional - sale_cost
                traded_notional += sale_notional
                transaction_cost += sale_cost
                ...
                and _is_first_eligible_session(self._close.index, index_position)
            ):
                ...
                if next_symbol != active_symbol:
                    ...
                    if next_symbol is not None and next_weight > 0.0:
                        purchase_notional = current_equity * next_weight
                        purchase_cost = (
                            purchase_notional * COST_PER_SIDE_BPS / 10_000.0
                        )
                        cash -= purchase_notional + purchase_cost
                        traded_notional += purchase_notional
                        transaction_cost += purchase_cost
                        units = purchase_notional / close_price
                        entry_price = close_price
                elif next_symbol is not None:
                    target_value = current_equity * next_weight
                    change_notional = target_value - position_value
                    if change_notional > 0.0:
                        added_units = change_notional / close_price
                        entry_price = _blended_entry_price(
                            float(entry_price), units, close_price, added_units
                        )
                        purchase_cost = (
                            change_notional * COST_PER_SIDE_BPS / 10_000.0
                        )
                        cash -= change_notional + purchase_cost
                        units += added_units
                        traded_notional += change_notional
                        transaction_cost += purchase_cost
                    elif change_notional < 0.0:
                        sale_notional = -change_notional
                        sale_cost = sale_notional * COST_PER_SIDE_BPS / 10_000.0
                        cash += sale_notional - sale_cost
                        units -= sale_notional / close_price
                        traded_notional += sale_notional
                        transaction_cost += sale_cost
                        if units <= 0.0:
                            units = 0.0
            ...
            cost = transaction_cost / starting_equity
            net_return = current_equity / starting_equity - 1.0
            daily_returns.loc[as_of] = net_return
            daily_turnover.loc[as_of] = turnover
            daily_costs.loc[as_of] = cost
            completed_equity = current_equity
            peak_completed_equity = max(peak_completed_equity, completed_equity)
            if active_symbol is not None:
                effective_exposure = (
                    units
                    * float(self._close[active_symbol].iloc[index_position])
                    / completed_equity
                    * PRODUCT_LEVERAGE_FACTORS.get(active_symbol, 0)
                )
                max_effective_exposure = max(
                    max_effective_exposure, effective_exposure
                )
...
def build_monthly_decision(
    ...
    return _monthly_decision_from_close(
        _normalized_ohlc_matrices(market_history)["close"],
        as_of=_normalize_session(as_of),
        account_equity=1.0,
        drawdown_scalar=1.0,
    )
...
def _frozen_ohlc_matrices(market_history: pd.DataFrame) -> dict[str, pd.DataFrame]:
    bars = _normalized_ohlc_matrices(market_history)
    if any(len(matrix) < REQUIRED_SESSIONS for matrix in bars.values()):
        raise ValueError(
            "frozen learning input requires at least 523 complete aligned SPY/BOXX sessions"
        )
    return {
        column: matrix.iloc[-REQUIRED_SESSIONS:]
        for column, matrix in bars.items()
    }


def _normalized_ohlc_matrices(
    market_history: pd.DataFrame,
    ...
    if any(
        ...
        raise ValueError(
            "frozen learning input permits only complete aligned SPY/BOXX sessions"
        )
    return {
        column: matrix.loc[:, [RISK_SYMBOL, DEFENSIVE_SYMBOL]]
        for column, matrix in bars.items()
    }
