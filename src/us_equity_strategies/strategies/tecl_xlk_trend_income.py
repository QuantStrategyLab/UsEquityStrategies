...
XLK_GATE_TIERED_BLEND_MODE = "xlk_gate_tiered_blend"
CORE_ASSETS = ("TECL", "XLK", "BOXX")
...
def _active_risk_asset_from_ratios(tecl_ratio: float, xlk_ratio: float) -> str:
    if tecl_ratio > 0.0 and xlk_ratio > 0.0:
        return "XLK+TECL"
    if tecl_ratio > 0.0:
        return "TECL"
    if xlk_ratio > 0.0:
...
def _resolve_tier_allocations(
    *,
    tier: str,
    full_tecl_ratio: float,
    mid_tecl_ratio: float,
    active_xlk_ratio: float,
    defensive_xlk_ratio: float,
) -> tuple[float, float, float, str]:
    if tier == "full":
        tecl_ratio = full_tecl_ratio
        xlk_ratio = active_xlk_ratio
        active_risk_asset = "XLK+TECL"
    elif tier == "mid":
        tecl_ratio = min(mid_tecl_ratio, full_tecl_ratio)
        xlk_ratio = active_xlk_ratio
        active_risk_asset = "XLK+TECL"
    elif tier == "defensive":
        tecl_ratio = 0.0
        xlk_ratio = defensive_xlk_ratio
        active_risk_asset = "XLK"
    else:
        raise KeyError(f"Unknown blend tier: {tier}")

    active_risk_ratio = tecl_ratio + xlk_ratio
    ...
    return tecl_ratio, xlk_ratio, boxx_ratio, active_risk_asset
...
def build_rebalance_plan(
    ...
    *,
    ...
    attack_allocation_mode=XLK_GATE_TIERED_BLEND_MODE,
    blend_gate_trend_source="XLK",
    blend_gate_tecl_weight=0.75,
    ...
    blend_gate_volatility_delever_retention_policy=POLICY_TECL_STEP_REBOUND_025_050,
    ...
    tecl_price = float(_indicator_value(indicators, "TECL", "price"))
    tecl_ma_trend = float(_indicator_value(indicators, "TECL", "ma_trend"))
    allocation_mode = str(attack_allocation_mode or XLK_GATE_TIERED_BLEND_MODE).strip().lower()
    if allocation_mode != XLK_GATE_TIERED_BLEND_MODE:
        raise ValueError("tecl_xlk_trend_income only supports xlk_gate_tiered_blend")
    ...
    tecl_entry_line = tecl_ma_trend * (1.0 + entry_buffer)
    tecl_exit_line = tecl_ma_trend * (1.0 - exit_buffer)

    trend_symbol = str(blend_gate_trend_source or "XLK").strip().upper()
    ...
    if trend_price is None:
        trend_price = tecl_price
    if trend_ma is None:
        trend_ma = tecl_ma_trend
    trend_entry_line = trend_ma * (1.0 + entry_buffer)
    trend_mid_line = trend_ma * (1.0 + mid_buffer)
    trend_exit_line = trend_ma * (1.0 - exit_buffer)
    current_blend_active = quantities.get("TECL", 0) > 0 or market_values.get("TECL", 0.0) > current_min_trade
    target_tecl_ratio = _as_clamped_ratio(blend_gate_tecl_weight, default=0.75)
    target_mid_tecl_ratio = _as_clamped_ratio(blend_gate_mid_tecl_weight, default=0.65)
    target_active_xlk_ratio = _as_clamped_ratio(blend_gate_active_xlk_weight, default=0.20)
    target_defensive_xlk_ratio = _as_clamped_ratio(blend_gate_defensive_xlk_weight, default=0.15)
    ...
    volatility_delever_redirect_symbol = str(
        blend_gate_volatility_delever_redirect_symbol or "XLK"
    ).strip().upper()
    if volatility_delever_redirect_symbol not in {"XLK", "BOXX"}:
        volatility_delever_redirect_symbol = "XLK"
    ...
    selected_tecl_ratio, selected_xlk_ratio, boxx_ratio, active_risk_asset = _resolve_tier_allocations(
        tier=blend_tier,
        full_tecl_ratio=target_tecl_ratio,
        mid_tecl_ratio=target_mid_tecl_ratio,
        active_xlk_ratio=target_active_xlk_ratio,
        defensive_xlk_ratio=target_defensive_xlk_ratio,
    )
    volatility_delever_triggered = (
        use_volatility_delever
        and selected_tecl_ratio > 0.0
    ...
    if volatility_delever_triggered:
        ...
        retained_tecl_ratio = selected_tecl_ratio * effective_retention_ratio
        volatility_delever_removed_ratio = max(0.0, selected_tecl_ratio - retained_tecl_ratio)
        selected_tecl_ratio = retained_tecl_ratio
        if volatility_delever_redirect_symbol == "BOXX":
            boxx_ratio += volatility_delever_removed_ratio
        else:
            selected_xlk_ratio += volatility_delever_removed_ratio
        if selected_tecl_ratio > 0.0 and selected_xlk_ratio > 0.0:
            active_risk_asset = "XLK+TECL"
        elif selected_tecl_ratio > 0.0:
            active_risk_asset = "TECL"
        elif selected_xlk_ratio > 0.0:
            active_risk_asset = "XLK"
        else:
        ...
        overlay_trigger_reasons.append(
            _translate_with_fallback(
                ...
                (
                    ...
                    f"({volatility_delever_threshold_detail}), redirect TECL to "
    ...
    if bool(market_regime_control_context["active"] and market_regime_control_route_allowed):
        before_market_regime_risk_ratio = selected_tecl_ratio + selected_xlk_ratio
        ...
        leverage_removed_ratio = max(0.0, selected_tecl_ratio * (1.0 - leverage_scalar))
        selected_tecl_ratio *= leverage_scalar
        if str(market_regime_control_context["route"]) == "risk_reduced":
            selected_xlk_ratio += leverage_removed_ratio
            market_regime_control_redirected_to_unlevered_ratio = leverage_removed_ratio
        if risk_asset_scalar < 1.0:
            selected_tecl_ratio *= risk_asset_scalar
            selected_xlk_ratio *= risk_asset_scalar
        after_market_regime_risk_ratio = selected_tecl_ratio + selected_xlk_ratio
        ...
        active_risk_asset = _active_risk_asset_from_ratios(selected_tecl_ratio, selected_xlk_ratio)
    ...
    tecl_target = core_equity * selected_tecl_ratio
    xlk_target = core_equity * selected_xlk_ratio
    deploy_ratio_text = f"{(selected_tecl_ratio + selected_xlk_ratio) * 100:.1f}%"
    if selected_tecl_ratio > 0.0:
        allocation_text = f"TECL {selected_tecl_ratio * 100:.1f}% + XLK {selected_xlk_ratio * 100:.1f}%"
    else:
        allocation_text = f"XLK {selected_xlk_ratio * 100:.1f}%"
    ...
    if overlay_trigger_count > 0:
        ...
        signal_context = {
            ...
            "fallback": (
                ...
                f"TECL {selected_tecl_ratio * 100:.1f}% + XLK {selected_xlk_ratio * 100:.1f}%"
            ),
            "params": {
                "trend_symbol": trend_symbol,
                "window": trend_ma_window,
                "tecl_ratio": f"{selected_tecl_ratio * 100:.1f}%",
                "xlk_ratio": f"{selected_xlk_ratio * 100:.1f}%",
            },
        }
    else:
        status_context = {
        ...
        signal_context = {
            ...
            "fallback": (
                f"{trend_symbol} below gated entry, hold defensive XLK {selected_xlk_ratio * 100:.1f}%"
            ),
            "params": {
                "trend_symbol": trend_symbol,
                "window": trend_ma_window,
    ...
    return {
        ...
        "limit_order_symbols": ("TECL", "XLK", *income_symbols),
        "portfolio_rows": (("TECL", "XLK"), income_symbols, ("BOXX",)),
        ...
        **income_layer_plan.diagnostics,
        ...
        "tecl_entry_line": tecl_entry_line,
        "tecl_exit_line": tecl_exit_line,
