...
**Platform-compatible status remains HOLD.** The next bounded offline gate is to replay this same synthetic first-open input through Schwab's existing dry-run-only execution adapter, with BOXX safe-haven cash sweep, integer risk buys, costs, settled funding and account-level share deltas explicitly reconciled. It must use guarded artificial ports and remain separate from any paper/live authority. A single matching example would still not validate the full window, lot ownership, dividend/settlement behavior or complete v2 option liabilities. Professional v1.1's second-platform extraction gate is not reached while first-platform applicability remains unverified.
