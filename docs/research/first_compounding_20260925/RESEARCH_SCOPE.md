...
The direct synthetic tests cover the budget cap/reference distinction, funding and whole-share execution, event/position continuity, private-input identity, and BOXX cash reservation, settlement, and prefix-only scenarios. The private historical summary binds each daily ledger by SHA-256; all 18 aligned paths reconcile security values, settled cash, unsettled proceeds, and receivables to NAV. Historical aggregate numbers in this document are not CI fixtures and CI does not read the private input bundle.

This is a stacked draft research change above UES PR #523. That PR's ordinary dependency still points to a pre-contract QPK revision; local tests using the fixed QPK research worktree do not establish ordinary dependency adoption or passing GitHub CI. QPK draft PR #637 and an explicit dependency update remain separate integration gates. This research PR is not a merge, release, or execution request.
