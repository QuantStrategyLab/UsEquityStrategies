...
decision log、equity curve、cash flows 和 candidate specs 写成
...
不改变生产策略默认行为。
每个场景还会写 `candidate_specs.csv`，按候选和参数逐行记录 family、rule type、signal
symbols、min history、parameter name/value，作为候选参数冻结证据。
