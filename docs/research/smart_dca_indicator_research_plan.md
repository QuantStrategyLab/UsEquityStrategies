...
| `ibit_smart_dca` AHR999 helper variants | `/home/ubuntu/Projects/dca_research_runs/ibit_helper_20260618/helper_matrix_diagnostics`; research handoff under `/home/ubuntu/Projects/dca_research_runs/ibit_helper_20260618/research_handoff_current` | do not promote helper variants; keep fixed DCA default and retain AHR999 baseline for explicit smart-mode comparison | guarded helper lowers skip/cash drag, but worst 2020-cycle scenario still underperforms fixed by `4.46%`; percentile helper directly underperforms fixed | full local artifacts exist; BTC cycle CSV is pinned by `research_export.v1`, source catalog, consumer contract registry, and research handoff/audit records documented in `docs/research/ibit_ahr999_helper_matrix_2026-06-18.md` |
...
当前固定研究候选集 `nasdaq_sp500_external_precomputed_variants` 只给外部
context 回测留入口，不改变生产等价候选或默认普通定投行为。它包含当前
`nasdaq_sp500_price_no_skip` baseline，以及两个预冻结的研究 variant：

- `nasdaq_sp500_precomputed_valuation_guard`：只使用 `cape_percentile`，当
  CAPE 处于高分位时把本期投入降到 `0.75x`，否则保持 `1.0x`。
...
无效日期、越界分位、重复日期、日期缺口和 `as_of` 截断。研究 CLI 在传入
...
- 价格路径优先使用 adjusted close / total return proxy；如果只能用 FRED `NASDAQ100`
  和 `SP500` price-only，报告必须标注不含分红和费用，并先通过
  `artifact_type=us_equity_price_proxy_research_csv` /
  `transform=us_equity.nasdaq_sp500.price_proxy.v1` 生成可审计输入。
...
这两个 helper 候选现在绑定
`research:ibit_btc_ahr999_helper_precomputed_variants` consumer contract；发布侧
必须提供 `BTC-USD.ahr999`、`BTC-USD.ahr999_365d_percentile` 和
`BTC-USD.ahr999_30d_slope`，策略研究 CLI 也会拒绝非正 AHR999、越界 percentile
或非有限 slope。
...
sha256、文件大小、候选集信号来源模式和 `compatible_signal_consumers`，方便后续复现实验、
证明没有临时更换数据，并能把 precomputed 候选和信号源仓库的 consumer contract 对齐。
对 Nasdaq/S&P price-only rerun，`input_csv` 应该指向信号源仓库导出的
`us_equity_price_proxy_research_csv`，并通过 `--price-manifest` 传入对应
...
- 当 CLI 传入 `--signal-source-family-catalog-manifest` 时，会校验 catalog manifest
  ...
  `scenario_manifest.json`，用于证明 precomputed 指标来自已发布且声明兼容的信号源家族目录。
...
min history、parameter name/value。precomputed IBIT 候选还会在 `candidate_summary.csv`
写出 `compatible_signal_consumers`，把研究候选和 `MarketSignalSources` 发布的
consumer contract 绑定，避免 AHR999-only / Mayer variant 在研究与生产信号契约之间漂移。
两者共同作为候选参数冻结和防过拟合证据。
...
若输入是 `MarketSignalSources` 导出的 Nasdaq/S&P price proxy CSV，直接把对应
manifest 作为 `--price-manifest` 传入：

```bash
python -m us_equity_strategies.backtests.smart_dca_research_cli \
  --signal-csv ./research_inputs/us_equity_price_proxy.csv \
  --trade-csv ./research_inputs/trade_prices.csv \
  --price-manifest ./research_inputs/us_equity_price_proxy.manifest.json \
  --output-dir ./artifacts/smart_dca_research/nasdaq_sp500_price_proxy \
  --candidate-set nasdaq_sp500_price_variants \
  --signal-columns QQQ,SPY \
  --trade-column close \
  --robustness-preset standard \
  --execution-days 1,10,15,20,25 \
  --sample-windows discovery:2017-06-20:2020-12-31,validation:2021-01-01:2023-12-31,oos:2024-01-01:2026-06-17 \
  --pretty
```
...
python -m us_equity_strategies.backtests.smart_dca_research_cli \
  ...
  --sample-windows discovery:2018-04-25:2020-12-31,validation:2021-01-01:2023-12-31,oos:2024-01-01:2026-06-18 \
...
python -m us_equity_strategies.backtests.smart_dca_research_cli \
  ...
  --research-signal-handoff-manifest ./research_inputs/research_handoff.json \
  --research-signal-handoff-consumer research:ibit_btc_ahr999_mayer_precomputed \
  --platform-signal-handoff-index ./research_inputs/platform_handoff_index.json \
  ...
  --robustness-preset standard \
  --execution-days 1,10,15,20,25 \
  --sample-windows validation:2021-01-01:2023-12-31,oos:2024-01-01:2026-06-18 \
  --start-dates 2018-04-25,2020-01-02,2021-01-04,2024-01-02 \
  --pretty
```
...
是否会改变结论。
使用 `MarketSignalSources` 导出的 research CSV 时，Nasdaq/S&P price proxy 输入应传
`--price-manifest`，precomputed signal 输入应传 `--signal-manifest`；CLI 会把上游
`research_export.v1`、transform、source version、列集合、日期范围和 CSV hash
校验结果写入 `scenario_manifest.json`。manifest 中出现疑似密钥、token、cookie
...
不一致时，研究运行会直接失败。price proxy CSV 必须具备正数 `QQQ` / `SPY`；precomputed
signal CSV 还必须日期有效、无重复、按原始行顺序单调递增，并且 manifest `as_of`
不早于 CSV `last_date`。若同时传入
`MarketSignalSources` 发布的 `market_signal_research_handoff.v1`，可用
`--research-signal-handoff-manifest` 让智能定投研究 CLI 在本次回测中直接校验上游
research handoff；该步骤会把 `research_export.v1`、source family catalog 和 consumer
contract registry 三个 manifest hash pin 成一条证据链，并验证 output CSV / quality
report 未被替换。若当前候选集兼容多个 `research:` consumer，必须用
`--research-signal-handoff-consumer` 指定本次审计的 consumer；若只兼容一个，CLI 会自动推断。
智能定投研究 CLI 仍应继续传入 `--signal-manifest`、`--signal-source-family-catalog-manifest`
和 `--signal-consumer-contract-registry-manifest`，并把同一组证据摘要写入
`scenario_manifest.json` 的 `research_signal_handoff_manifest` 记录。
若同时传入
`--signal-source-family-catalog-manifest`，CLI 还会记录 catalog manifest、catalog hash、
family count、known-family coverage、matched family 和 consumer-contract coverage 状态，
并要求 catalog 里至少一个 family 同时匹配 transform 与当前 candidate set 的
`compatible_signal_consumers`，让研究结果能追溯到 `MarketSignalSources` 发布的信号家族目录。若同时传入
...
`market_signal_platform_handoff.v1` 交接清单对齐。若传入
`--platform-signal-handoff-index`，CLI 会先按当前 candidate set 的
`compatible_signal_consumers`、`derived_indicators`、`--end-date` 前的 `as_of` 和 fresh
状态解析最新匹配 handoff manifest，再复用同一套 handoff / linked manifest 校验，并把
index schema、handoff count、resolved handoff path 和 SHA-256 写入
`scenario_manifest.json`。这允许平台只发布一个
`market_signal_platform_handoff_index.v1`，研究任务自动选取当期可消费的交接清单。
若平台部署流程已经保存 `market_signal_consumption_audit.v1`，可同时传入
`--signal-consumption-audit-json`。研究 CLI 会用当前 candidate set 的 runtime consumer
校验这份 deploy/startup 决策记录、linked runtime signal bundle manifest SHA-256、
bundle identity、runtime payload path 和 `all_runtime_consumers_covered`，并把摘要写入
`scenario_manifest.json` 的 `signal_consumption_audit` 记录。该记录用于证明研究矩阵所引用的
平台运行时信号和部署门禁使用同一条 artifact 证据链。
若同时传入 `--require-runtime-consumer-coverage`，研究 CLI 会要求 source catalog 或
handoff / consumption-audit 链路证明 `all_runtime_consumers_covered=true`，并把该结果写入
`scenario_manifest.json`，防止研究用的信号目录和平台 runtime consumer 覆盖门禁脱节。
...
和 `ahr999` 字段，适合当前生产 IBIT 智能定投 baseline；而
...
设置 `--sample-windows` 时，CLI 会把每个 `label:start:end` 固定样本窗口作为独立
robustness 维度，例如 discovery / validation / oos 或 2020 / 2022 / IBIT-live stress
window。窗口不会触发任何参数搜索，只会把同一组冻结候选重复跑在指定日期段内；如果同时
设置 `--start-dates`，每个 rolling start 会被限制在对应 sample window 的起止日期内。
`scenario_coverage.csv` 会记录 `scenario_sample_window_labels` 和
`scenario_sample_window_label_count`，`selection_summary.csv` 会复制这些字段为
`matrix_scenario_sample_window_*`，让平台或研究看板能确认样本外窗口确实参与了 promotion gate。
`--cadences` 默认只跑 `monthly`；设置为 `weekly,monthly,quarterly` 后会把 cadence 纳入
矩阵。也可以用 `--robustness-preset standard` 自动填充
`weekly,monthly,quarterly` 和 `500,1000,3000` 三档月度基准金额，再按需要覆盖
`--execution-days` 或 `--start-dates`。周投金额按 `monthly_contribution_usd * 12 / 52`
折算，季投金额按 `monthly_contribution_usd * 3` 折算，用同一个月度基准值近似保持
年化现金流可比。
