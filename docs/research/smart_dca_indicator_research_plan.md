...
用于比较 fixed DCA 与 `nasdaq_sp500_price_defensive`、`ibit_btc_ahr999_mayer_cycle` 等
...
行；`write_research_artifacts` 会把 metrics、evaluation summary 和 decision log 写成
...
独立 CSV、`run_manifest.json` 和顶层 `scenario_index.csv` / `scenario_manifest.json`，
其中 manifest 记录每个 CSV 的 sha256。这个 helper 是后续批量数据回测和结果 CSV 的基础，
不改变生产策略默认行为。
