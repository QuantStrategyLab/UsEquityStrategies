...
python -m us_equity_strategies.signals.signal_bundle_cli \
  --research-handoff-manifest ./research_inputs/research_handoff.json \
...
`MarketSignalSources` 发布的 `market_signal_research_handoff.v1`，应先用
`us_equity_strategies.signals.signal_bundle_cli --research-handoff-manifest` 做上游
交接校验；该步骤会把 `research_export.v1`、source family catalog 和 consumer contract
registry 三个 manifest hash pin 成一条证据链，并验证 output CSV / quality report 未被替换。
智能定投研究 CLI 仍应继续传入 `--signal-manifest`、`--signal-source-family-catalog-manifest`
和 `--signal-consumer-contract-registry-manifest`，把同一组证据摘要写入
`scenario_manifest.json`。
若同时传入
