...
`research_export.v1` manifest stay beside the scenario artifacts as the raw
input hash chain.
...
`us_equity_price_proxy_research_csv`，并把对应 manifest 随场景目录一起保存；这样即使
策略 CLI 当前不把 price-only manifest 作为 `--signal-manifest` 校验对象，也能追溯
FRED 原始快照、输出 CSV 和回测场景三层 hash。
...
不一致时，研究运行会直接失败。precomputed signal CSV 还必须日期有效、无重复、
按原始行顺序单调递增，并且 manifest `as_of` 不早于 CSV `last_date`。若同时传入
