...
顶层 `index.json` 使用 `market_signal_index.v1`，列出可供平台选择的 manifest 相对路径、
manifest sha256、bundle id、as_of、canonical input 和 freshness status；平台可按目标 as_of
选择最新 fresh entry，先校验 manifest sha256，再对 manifest 和 bundle 做完整校验。
...
本仓库也提供一个最小命令行审计入口，平台或人工验收可以在注入策略前先校验 manifest
和 bundle hash，并输出非敏感审计字段：
