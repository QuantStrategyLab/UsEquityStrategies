...
这 9 条 profile 是当前策略目录里的 `runtime_enabled` `us_equity` profiles。平台能否实际启用由各平台能力矩阵和部署配置决定；能切换不等于适合直接上实盘。
...
| `dynamic_mega_leveraged_pullback` | Mega 2x 回调策略 | feature snapshot + 日频行情 | 动态 mega-cap 池里选 top3 的 2x 产品，用 QQQ 200SMA/ATR 风控，剩余停 BOXX。 | 可切换但暂不建议 live；MAGS/TACO 未合并到正式运行逻辑。 |
