...
下面是 2026-04-18 通过 Cloud Run env 核对到的当前状态。

| 平台服务 | 当前 profile | 运行说明 |
| --- | --- | --- |
| IBKR `interactive-brokers-quant-service` | `soxl_soxx_trend_income` | 半导体 SOXL/SOXX 趋势收益线；`ACCOUNT_GROUP=default`，实际 paper/live 由账号组 secret 决定。 |
| Schwab `charles-schwab-quant-service` | `tqqq_growth_income` | TQQQ 增长收益线；当前设置 `DUAL_DRIVE_UNLEVERED_SYMBOL=QQQM`，信号仍看 `QQQ`；挂载 `crisis_response_shadow`，模式为 `shadow`，只进通知/日志，不改交易。 |
| LongBridge HK `longbridge-quant-hk-service` | `tech_communication_pullback_enhancement` | 科技通信回调增强；使用 feature snapshot artifact；当前 `LONGBRIDGE_DRY_RUN_ONLY=false`。 |
| LongBridge SG `longbridge-quant-sg-service` | `soxl_soxx_trend_income` | 半导体 SOXL/SOXX 趋势收益线。 |

切换策略时只改对应平台的 `STRATEGY_PROFILE` 和配套 snapshot/config env；不要改 service name。详细切换步骤看 `QuantPlatformKit/docs/us_equity_live_switch_runbook.zh-CN.md`。
...
| `tqqq_growth_income` | TQQQ 增长收益 | 直接运行输入 | `QQQ` / `TQQQ` 双轮增长，默认 `45% / 45% / 8% BOXX / 2% cash`；`QQQM` 可作为低单价交易代理。 | Schwab 当前实盘线；小账户最容易落地。 |
| `soxl_soxx_trend_income` | SOXL/SOXX 半导体趋势收益 | 直接运行输入 | 以 `SOXX` 140 日趋势闸门控制 `SOXL` / `SOXX` / `BOXX`；剩余资金停 BOXX，可叠加收入层。 | IBKR、LongBridge SG 当前线；波动高于宽基。 |
| `tech_communication_pullback_enhancement` | 科技通信回调增强 | feature snapshot | 科技/通信个股月频选择，受控回调入场，保留 BOXX 缓冲。 | LongBridge HK 当前线；需要月度 snapshot。 |
...
| --- | --- | ---: | ---: | --- | --- |
| `tqqq_growth_income` 的可执行近似版 `video_like_pullback_next_close`，5 bps 单边成本 | 2017-01-03 至 2026-04-10 | 33.96% | -31.48% | 明显优于 QQQ CAGR，回撤接近 QQQ；远低于 TQQQ 买入持有的 -81.66% 回撤。当前 live 版在此基础上加入动态波动率回调门槛，因此这是近似证据，不是逐行完全相同的 live 回测。 | `InteractiveBrokersPlatform/research/results/video_qqq_tqqq_dual_drive_comparison.csv` |
...
- `soxl_soxx_trend_income`：当前 live 逻辑和测试覆盖完整，但本仓库没有归档一份可直接复查的长期 backtest summary。历史口头结果需要重新跑并提交 summary 后才能写成正式指标。
...
| 研究方向 | 当前状态 | 不直接实盘的原因 |
| --- | --- | --- |
| `crisis_response_shadow` 插件 | Schwab 当前以 `shadow` 模式挂载到 `tqqq_growth_income`，只写信号、日志和通知上下文。 | 现在是 defense-only 黑天鹅观察流，不下单、不改 allocation；需要 20 个交易日以上稳定 shadow 日志后再做 evidence review。 |
