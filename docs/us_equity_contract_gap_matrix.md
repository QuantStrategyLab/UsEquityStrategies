...
That means the original profile-by-profile platform gaps for the runtime-enabled strategies are closed. The remaining work is about payload normalization, artifact discipline, and future platforms or strategies.
...
All eight runtime-enabled profiles now use canonical `required_inputs` names at the strategy boundary. P4 still needs to converge platform input builders and payload shapes onto the same vocabulary.
